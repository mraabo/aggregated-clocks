...
(defun aggregated-clock--insert-tree (tree level)
  ...
  (let ((sorted-keys (sort (hash-table-keys tree) #'string<)))
    ...
    (dolist (file files)
      ...
      (let ((buf (find-file-noselect file)))
	    ...
	    ;; Find org headings
	      ...
	      ;; Get heading level, full outline path and char position where entry ends
              (let* ((level (org-current-level))
                ...
                (save-excursion
		  ;; Search for LOGBOOK environments with clock entries
			...
			;; Store clock entries in activity-data
                        (push clock-content
                              (gethash full-path activity-data))
                        ;; Store outline path in hierarchy-tracker
           ...
           (dotimes (i (length outline-path))
             (let* ((heading (nth i outline-path))
               ...
               (if is-last
                 ...
                 (unless (gethash heading current-level)
                   (puthash heading (make-hash-table :test 'equal) current-level))
                 (setq current-level (gethash heading current-level)))))))
       activity-data)
